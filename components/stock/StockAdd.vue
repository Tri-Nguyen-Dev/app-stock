...
.addStock
	...
	.addStockContent.p-3
		...
		.p-input-icon-right.w-full
			...
			InputText.w-full(v-model='stockInformation.barCode')
		h5.mb-1 SKU
		InputText.w-full(v-model='stockInformation.sku')
		h5.mb-1.required__title Name
		InputText.w-full(
			v-model='stockInformation.name',
			:class='{ "name--error": $v.stockInformation.name.$error }'
		)
		.error-message(
			v-if='$v.stockInformation.name.$dirty && !$v.stockInformation.name.required'
		...
		Dropdown.w-full(
			...
			:showClear='true',
			:class='{ "category--error": $v.stockInformation.category.$error }'
		)
			template(#value='slotProps')
		.error-message(
			v-if='$v.stockInformation.category.$dirty && !$v.stockInformation.category.required'
		...
		.grid
			.col-6
				...
				InputText.w-full(
					type='number',
					:min='1',
					v-model='stockInformation.quantity',
			...
			.col-6
				...
				.error-message(
					v-if='$v.stockInformation.unit.$dirty && !$v.stockInformation.unit.required'
				) Please choose unit!
		.grid
			.col-6
				h5.mb-1 Weight
				.p-input-icon-right.w-full
					.icon.icon--right Kg
					InputText.w-full(type='number', :min='1', v-model='stockInformation.weight')
					.error-message(v-if='stockInformation.weight < 0') Weight cannot be negative!
			.col-6
				h5.mb-1 Value
				.p-input-icon-right.w-full
					.icon.icon--right $
					InputText.w-full(type='number', :min='1', v-model='stockInformation.value')
		...
		.grid.mb-3
			.col
				...
				.p-input-icon-right.w-full
					.icon.icon--right cm
					InputText.w-full(type='number', :min='1', v-model='stockInformation.length')
					.error-message(v-if='stockInformation.length < 0') Length cannot be negative!
			.col
				h5.mb-1 Width
				.p-input-icon-right.w-full
					.icon.icon--right cm
					InputText.w-full(type='number', :min='1', v-model='stockInformation.width')
					.error-message(v-if='stockInformation.width < 0') Width cannot be negative!
			.col
				h5.mb-1 Height
				.p-input-icon-right.w-full
					.icon.icon--right cm
					InputText.w-full(type='number', :min='1', v-model='stockInformation.height')
